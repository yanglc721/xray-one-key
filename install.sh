...
#           VLESS-XTLS-Vision / SOCKS5 / SS2022 / Trojan / 
#           Snell v4 / Snell v5 / AnyTLS / NaïveProxy (共12种)
...
init_db() {
    ...
    if jq -n --arg v "4.0.0" --arg t "$now" \
      '{version:$v,xray:{},meta:{created:$t,updated:$t}}' >"$tmp" 2>/dev/null; then
        mv "$tmp" "$DB_FILE"
        return 0
    fi
    # jq 失败时使用简单方式创建
    echo '{"version":"4.0.0","xray":{},"meta":{}}' > "$DB_FILE"
    rm -f "$tmp"
    return 0
}
...
declare -A SVC_PROC=(
    ...
    [vless-nnell-shadowtls]="shadow-tls"
    ...
    case "$protocol" in
        vless-ws|vmess-ws)
            ...
            if [[ "$has_master" == "true" ]]; then
                ...
                # 独立运行时才需要端口，优先 18443
                if ! ss -tuln 2>/dev/null | grep -q ":18443 " && ! is_internal_port_occupied "18443" >/dev/null; then
                    echo "18443"
                else
                    gen_port
                fi
...
_refresh_core_versions_now() {
    ...
    _check_version_updates_async "$xray_current"
...
update_core_menu() {
    while true; do
        _header
        echo -e "  ${W}核心版本管理 (Xray/Snell v5)${NC}"
        _line
        _show_core_versions
        _line
        ...
        case "$choice" in
            ...
            2) _update_core_with_channel_select "Snell v5" "surge-networks/snell" "snell-server-v5" "vless-snell-v5" "install_snell_v5" ;;
...
select_protocol() {
    ...
    _item "0" "返回"
    echo ""
    echo -e "  ${D}提示: 5/6 默认占用18443端口，3/4 可作为回落共用${NC}"
    echo ""
    
    while true; do
        read -rp "  选择协议 [0-12]: " choice
        case $choice in
...
main_menu() {
    ...
    while true; do
        ...
        echo -e "  ${D}核心: Xray ${xray_ver_with_status}${NC}"
        echo ""
        ...
        if [[ -n "$installed" ]]; then
            case $choice in
                ...
                6) manage_protocol_services; skip_pause=true ;;
                7) manage_routing; skip_pause=true ;;
                8) manage_cloudflare_tunnel; skip_pause=true ;;
